...
void init_simcom(uart_port_t uart_num, int tx_num, int rx_num, int baud_rate)
{
  ...
  // Set UART pins
  uart_set_pin(uart_num, tx_num, rx_num, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
  // Install UART driver
  uart_driver_install(uart_num, BUF_SIZE, BUF_SIZE, 0, NULL, 0);
  // ham xu ly ngat khi nhan duoc data
  xTaskCreate(UART_ISR_ROUTINE, "uart_echo_task1", 4096, NULL, 10, NULL);
}

// ham xu ly ngat khi nhan duoc data -> luu data
void UART_ISR_ROUTINE(void *pvParameters)
{
  uint8_t data[512];
  while (1)
  {
    int len = uart_read_bytes(NUMERO_PORTA_SERIALE2, data, (512 - 1), 100 / portTICK_RATE_MS);
    // ghi data vao AT_buffer
    if (len)
    {
      data[len] = '\0';
      ESP_LOGI(TAG, "Rec: %s", (char *)data);
      if (strstr((char *)data, "+CMQPUB:")) // kiem tra data nhan duoc co "CMQPUB" hay khong
      {
        simcom_7020.mqtt_CB((char *)data);
      }
      else
      {
        memcpy(simcom_7020.AT_buff, data, len); // copy data nhan duoc vao simcom_AT_buffer
...
static void send_ATComand(char *ATcommand)
{
  ...
  uart_write_bytes(NUMERO_PORTA_SERIALE2, (char *)ATcommand, strlen((char *)ATcommand));
  uart_write_bytes(NUMERO_PORTA_SERIALE2, "\r\n", strlen("\r\n"));
  vTaskDelay(100 / portTICK_RATE_MS);
}
...
// kiem tra Feedback va (time < timeout)
AT_flag _readFeedback(uint32_t timeout, char *expect)
{
  uint64_t timeCurrent = esp_timer_get_time() / 1000; // lay thoi gian tu khi chip wakeup or khoi dong
  while (esp_timer_get_time() / 1000 < (timeout + timeCurrent))
  {
    vTaskDelay(10 / portTICK_RATE_MS);
    if (simcom_7020.AT_buff_avai)
    {
      if (strstr((char *)simcom_7020.AT_buff, "ERROR"))
      {
        vTaskDelay(1000 / portTICK_RATE_MS);
        return AT_ERROR;
      }
      else if (strstr((char *)simcom_7020.AT_buff, expect))
        return AT_OK;
    }
  }
  return AT_TIMEOUT;
...
// kiem tra AT_buff co du lieu khong va (time < timeout)
bool _readSerial(uint32_t timeout)
{
  ...
  if (simcom_7020.AT_buff_avai == false)
    return false;
  else
    return true;
}
...
bool isRegistered(int retry)
{
  while (retry--)
  {
    ...
    if (_readSerial(1000) == false)
      continue;
    if (strstr((char *)simcom_7020.AT_buff, "0,1") || strstr((char *)simcom_7020.AT_buff, "0,5") || strstr((char *)simcom_7020.AT_buff, "1,1") || strstr((char *)simcom_7020.AT_buff, "1,5") || strstr((char *)simcom_7020.AT_buff, "2,1") || strstr((char *)simcom_7020.AT_buff, "2,5"))
      return true;
    else
      continue;
  }
...
void Check_network()
{
  ...
  POWER_ON:
  if (isInit(4))
    ESP_LOGW(TAG, "module connect physical sucess");
  else
    ESP_LOGW(TAG, "module connect physical fail");
  FINE_NET:
  res = isRegistered(10);
  if (res)
    ESP_LOGW(TAG, "Module registed OK");
  else
  {
    if(!isInit(3))
    {
     ESP_LOGW(TAG, "Module registed FALSE");
     goto POWER_ON;
    }
    else{
      goto FINE_NET;
...
{
  if (!mqtt_new(clientMQTT, 50000, 512, 1))
    return false;
  AT_flag res;
  char buf[300];
